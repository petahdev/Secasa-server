...
function sendEmailToAll($recipients, $name, $position, $message) {
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = "New Admin Message from SECASA (St. Monica)";
        $mail->Body    = "
        <h2>Message from SECASA Admin (St. Monica)</h2>
        ...
        $mail->send();
        echo "<div style='color: green; text-align:center;'>Message sent successfully to St. Monica members!</div>";
    } catch (Exception $e) {
...
function getStPaulEmails($conn) {
    $emails = [];
    $sql = "SELECT email FROM stmonica";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($recipients)) {
        ...
        echo "<div style='color: red; text-align:center;'>No emails found for St. Monica members.</div>";
    }
}
